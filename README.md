...
> 本仓库是 [AgentChat](https://github.com/Shy2593666979/AgentChat) 的二次开发版本。整体架构与技术选型沿袭原项目，改动聚焦在**模型接入层解耦、Agent 编排增强、附件真实理解与安全收尾**四个方面。

### 1. 模型接入层（去阿里化、模型可插拔）

原项目默认内置阿里百炼（DashScope）Qwen 系列模型并硬编码 API Key，本仓库改为**用户自带模型**，支持任意 OpenAI 兼容模型（Kimi / DeepSeek / 智谱 等），由 `ModelManager` 统一管理会话模型与工具调用模型。

| 模型位置 | 原项目 | 本仓库 |
| --- | --- | --- |
| 对话模型 | qwen-plus（阿里百炼） | kimi-k2.6（示例，可替换） |
| 工具调用模型 | qwen-plus（阿里百炼） | kimi-k2.6（示例，可替换） |
| 推理模型 | deepseek-reasoner | kimi-k3（示例，可替换） |
| Embedding | text-embedding-v4（阿里） | embedding-3（智谱） |
| Rerank | gte-rerank-v2（阿里） | 可配置 |

### 2. 灵寻（LingSeek）Agent 交互增强

- **单节点重试**：任务执行中某个节点失败或结果不满意时，可定位到该节点单独重跑，并基于更新后的全部步骤结果重新生成最终答案。
- **暂停 / 停止**：流式执行过程中可随时中断。
- **节点详情**：展示每个节点的完整执行日志（思考、目标、工作流、注意事项等）。
- **复制为新任务**：将历史会话的完整步骤定义复制为新任务重新执行。

### 3. 工作台附件上传与理解

- 修复「发送旁附件按钮」上传失效的问题，接入 MinIO 对象存储实现真实上传。
- 新增后端文本抽取能力（`services/workspace/file_attachment.py`）：签名 URL → 还原对象 Key → 下载 → `doc_parser` 解析（PDF / docx / txt / markdown）→ 注入提示词，让模型真正「读懂」附件，消除幻觉输出 `<tool_calls>` 假工具调用的问题。

### 4. 安全与工程规范

- 将含 API Key 的 `config.yaml` 移出版本控制，改用脱敏模板 `config.yaml.example` 配合 `.gitignore`，杜绝密钥泄露。
- 对 `docker_config.yaml` 中的个人标识进行脱敏。

### 5. Bug 修复

- 修复知识库上传失败：`knowledge_file.oss_url` 由 `VARCHAR(255)` 改为 `TEXT`，解决 MinIO 签名 URL 超长（约 330 字符）导致写入失败的问题。
